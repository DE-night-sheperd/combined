...
class DeadlockSecurityMonitor:
    ...
    def get_active_users(self):
        ...
        try:
            ...
            try:
                wmi_c = wmi.WMI()
                logon_sessions = wmi_c.Win32_LogonSession()
                for session in logon_sessions:
                    try:
                        for assoc in session.associators(wmi_association_class="Win32_LoggedOnUser"):
                            username = assoc.Name
    ...
    def lock_system(self):
        ...
        self.disable_camera()
        self.log_event("SYSTEM LOCKED - security breach detected")
    ...
    def show_token_unlock_dialog(self):
        root = Tk()
        root.title("DEADLOCK - SYSTEM LOCKED")
        root.geometry("600x300")
        root.configure(bg="#000000")
        root.resizable(False, False)
        
        Label(
            root, text="⚠️  SYSTEM LOCKED - TOO MANY FAILED ATTEMPTS", 
            fg="#ff0000", bg="#000000", font=("Courier New", 14, "bold"), wraplength=550
        ).pack(pady=30)
        
        Label(
            root, text="Please insert your DEADLOCK USB Token to unlock.", 
        ...
        def check_token():
            ...
            for drive in usb_drives:
                success, msg = self.usb_key.verify_peripheral_key(drive)
                if success:
                    verified = True
                    break
            if verified:
                self.log_event("Token verified - system unlocked")
                self.locked = False
                self.auth_attempts = 0
                root.destroy()
        ...
        Button(
            root, text="CHECK USB TOKEN", command=check_token,
            bg="#008000", fg="#ffffff", font=("Courier New", 12)
        ).pack(pady=20)
