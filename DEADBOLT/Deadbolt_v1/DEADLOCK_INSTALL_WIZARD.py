...
class DeadlockInstallWizard:
    def __init__(self, root):
        self.root = root
        self.root.title("Deadlock - Setup Wizard")
    ...
    def setup_ui(self):
        ...
        tk.Label(
            title_frame,
            text="  Deadlock Endpoint Shield - Setup",
    ...
    def show_usb_insertion(self):
        ...
        self.usb_listbox = tk.Listbox(self.content_frame, font=("MS Sans Serif", 9), height=6)
        ...
        tk.Button(
            btn_frame,
            text="Skip USB (Limited Functionality)",
            bg="#ffaa00",
            fg="#000000",
            relief=tk.RAISED,
            bd=2,
    ...
    def skip_usb(self):
        if messagebox.askyesno(
            "Skip USB?",
            "Are you sure you want to skip USB key generation?\n\n"
            "This will limit some security features, but the rest\n"
            "of the system will still function normally.",
            parent=self.root
        ):
            self.selected_usb = None
            self.skip_usb_key = True
            if self.current_step < len(self.steps) - 1:
                self.current_step += 2
                self.show_current_step()
    ...
    def show_token_setup(self):
        if hasattr(self, 'skip_usb_key') and self.skip_usb_key:
            self.current_step += 1
            self.show_current_step()
            return
    ...
    def show_key_generation(self):
        if hasattr(self, 'skip_usb_key') and self.skip_usb_key:
            if hasattr(self, 'token1_var') and hasattr(self, 'token2_var'):
    ...
    def start_key_generation(self):
        ...
        self.gen_progress_index = 0
        self.gen_total_steps = 6
        
        def keygen_progress_callback(step_index, total_steps, message):
            self.progress_label.config(text=message)
            progress_pct = int((step_index / (total_steps - 1)) * 100)
            self.progress_bar['value'] = progress_pct
            self.progress_percent.config(text=f"{progress_pct}%")
            self.root.update()
        
        import threading
        def run_keygen():
            success, msg = self.usb_key.generate_peripheral_key(
                self.selected_usb,
                progress_callback=keygen_progress_callback
            )
            self.root.after(0, lambda: self.finish_key_generation(success, msg))
        
        threading.Thread(target=run_keygen, daemon=True).start()
        
    def finish_key_generation(self, success, msg):
        if success:
    ...
    def show_complete(self):
        ...
        tk.Label(
            self.content_frame,
            text="Deadlock Endpoint Shield has been successfully set up!",
            font=("MS Sans Serif", 10),
            bg=WIN98_BG,
            fg=WIN98_BLACK
        ).pack(pady=5)
        ...
        if hasattr(self, 'skip_usb_key') and self.skip_usb_key:
            tk.Label(
                self.content_frame,
                text="\n⚠️  USB Peripheral Key NOT generated",
                font=("MS Sans Serif", 10, "bold"),
                bg=WIN98_BG,
                fg="#ff8800"
            ).pack(pady=2)
            ...
            tk.Label(
                self.content_frame,
                text="\nYou can generate a USB key later via the admin panel.",
                font=("MS Sans Serif", 9),
                bg=WIN98_BG,
                fg="#666666"
            ).pack(pady=10)
...
def main():
    root = tk.Tk()
    app = DeadlockInstallWizard(root)
    root.mainloop()
