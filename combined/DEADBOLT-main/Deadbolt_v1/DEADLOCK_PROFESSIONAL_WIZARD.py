...
print("=== DEADLOCK ENDPOINT SHIELD - PROFESSIONAL SETUP WIZARD ===")

class ModernInstallerWizard:
    def __init__(self, root):
        self.root = root
        self.root.title("Deadlock Endpoint Shield v1.0 Setup")
        ...
        self.install_path = os.path.join(os.environ["LOCALAPPDATA"], "DeadlockEndpointShield")
        self.pages = [
            "Welcome",
            "License Agreement",
            "Installation Folder",
    ...
    def welcome_page(self):
        ttk.Label(
            self.content_frame,
            text="Welcome to the Deadlock Endpoint Shield Setup Wizard",
            style='Title.TLabel'
        ).pack(anchor=tk.W, pady=(0, 20))
        
        ttk.Label(
            self.content_frame,
            text="This wizard will guide you through the installation of Deadlock Endpoint Shield v1.0.",
...
Copyright (c) 2026"""
    ...
    def folder_page(self):
        ...
        ttk.Label(
            self.content_frame,
            text="Choose the folder in which to install Deadlock Endpoint Shield.",
            style='Subtitle.TLabel'
        ).pack(anchor=tk.W, pady=(0, 20))
        ...
        ttk.Label(
            self.content_frame,
            text="Setup will install Deadlock Endpoint Shield to the following folder.\nTo continue, click Next.",
            style='Wizard.TLabel'
    ...
    def ready_page(self):
        ...
        tk.Label(
            info_frame,
            text=f"• Secret Key Offset: {self.secret_offset}",
            bg='#ffffff',
            font=('Segoe UI', 9)
        ).pack(anchor=tk.W, pady=3)
    ...
    def install_page(self):
        ttk.Label(
            self.content_frame,
            text="Installing Deadlock Endpoint Shield",
            style='Title.TLabel'
        ).pack(anchor=tk.W, pady=(0, 30))
    ...
    def perform_installation(self):
        ...
        try:
            # Use saved variables (already stored when leaving previous pages)
            # No need to access widgets here!
            ...
            source_drivers = os.path.join(current_dir, "Deadbolt_Drivers")
            dest_drivers = os.path.join(self.install_path, "Drivers")
            
            if os.path.exists(source_drivers):
                shutil.copytree(source_drivers, dest_drivers, dirs_exist_ok=True)
            
            # Copy all Python files
            for file in os.listdir(current_dir):
                if file.endswith(".py") and not file.startswith("DEADLOCK_PROFESSIONAL_WIZARD"):
    ...
    def complete_page(self):
        ttk.Label(
            self.content_frame,
            text="Completing the Deadlock Endpoint Shield Setup Wizard",
            style='Title.TLabel'
        ).pack(anchor=tk.W, pady=(0, 20))
        
        ttk.Label(
            self.content_frame,
            text="Deadlock Endpoint Shield has been successfully installed.",
            style='Subtitle.TLabel'
        ).pack(anchor=tk.W, pady=(0, 30))
        ...
        ttk.Checkbutton(
            self.content_frame,
            text="Launch Deadlock Endpoint Shield",
    ...
    def next_page(self):
        # Save values before leaving page 2 (folder) and 3 (config)
        if self.current_page == 2:
            if hasattr(self, 'path_entry') and self.path_entry.winfo_exists():
                self.install_path = self.path_entry.get()
        elif self.current_page == 3:
            if hasattr(self, 'offset_entry') and self.offset_entry.winfo_exists():
                try:
    ...
    def cancel(self):
        if messagebox.askyesno("Cancel Setup", "Are you sure you want to cancel Deadlock Endpoint Shield Setup?"):
            self.root.destroy()
            
    def finish(self):
        self.root.destroy()
        if self.launch_var.get():
            print("Launching Deadlock...")
            messagebox.showinfo("Deadlock Endpoint Shield", "Setup complete! Deadlock is ready to use!")
